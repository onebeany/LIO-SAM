...
enum class SensorType { VELODYNE, OUSTER, LIVOX, ML_X };
...
public:
    ...
    ParamServer()
    {
        ...
        else if (sensorStr == "ml_x")
        {
            sensor = SensorType::ML_X;
        }
        else
        {
            ROS_ERROR_STREAM(
                "Invalid sensor type (must be either 'velodyne' or 'ouster' or 'livox or 'ml_x'): " << sensorStr);
            ros::shutdown();
        }
